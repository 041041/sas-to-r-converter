        ...
        if dialect == "Modern R (dplyr)":
            ...
            for v in num_vars:
                for s in stats:
                    ...
                    if grp_vars:
                        ...
                        formula = 'as.formula(paste(var, "~", grp))'
            ...
            if len(agg_dfs) > 1:
                if grp_vars:
                    ...
                    merge_cols = "NULL"
                lines.append(
                f"{out} <- Reduce(function(a,b) merge(a,b,by={merge_cols}), list({', '.join(agg_dfs)}))"
                )
            elif agg_dfs:
...
class LLMConverter:
    ...
    def convert(self, ir: MacroIR, dialect: str) -> tuple[str, float]:
        ...
        prompt = (
            ...
            f"3. Use df[[param]] for dynamic column references\n"
            f"4. Dataset name parameters → dataframe arguments\n"
            f"5. PROC steps → equivalent R/dplyr code inside function. "
            f"For PROC SORT use df[order(df[[by_col]]), ] — NEVER bare column names\n"
            f"5b. For PROC TRANSPOSE use tidyr::pivot_longer() with dynamic column refs\n"
