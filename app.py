...
for key, default in {"sas_input": "", "upload_key": 0, "uploaded_csvs": {}}.items():
...
# 🌟 FEATURE 1: Excel Support Added Here
def safe_read_file(file_obj, filename):
    """Attempts to read CSV or Excel gracefully."""
    if filename.lower().endswith(('.xlsx', '.xls')):
        try:
            file_obj.seek(0)
            return pd.read_excel(file_obj)
        except Exception as e:
            raise RuntimeError(f"Could not parse Excel. Error: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(f"Could not parse CSV. Error: {str(e)}")
...
def clean_r_code(text):
    ...
    if "pivot_longer" in cleaned:
        ...
        cleaned = f'df <- {source_table} %>%\n  pivot_longer(cols = c({cols}),\n               names_to = "{names_to}",\n               values_to = "{values_to}")\ndf'

    is_freq_step = "count(" in cleaned and "merge(" not in cleaned and "join(" not in cleaned and ("pivot_wider" in cleaned or "rename(COUNT" not in cleaned)
    if is_freq_step:
        ...
        if match:
            ...
            cleaned = re.sub(r"df\s*<-\s*\w+\s*%>%.*", f'df <- {source_table} %>%\n  count({vars}) %>%\n  rename(COUNT = n)', cleaned, flags=re.DOTALL)
...
def call_llm_api(step, df_cols, env_names=None, dialect="Base R"):
    ...
    input_context = f"A dataframe named 'df' with columns: {df_cols}" if df_cols else "Convert this step. You have access to the tables listed below."

    if dialect == "Modern R (dplyr)":
        rule_set = (
            "1. Use modern R (tidyverse). Use the pipe operator (%>%) for chaining.\n"
            "2. IF SAS uses DATALINES: ONLY create the data.frame using `data.frame(...)`. STOP immediately.\n"
            "3. IF SAS reads an existing table: start the pipeline exactly with `df <- TABLE_NAME %>%`.\n"
            "4. FOR DATA STEPS: Create new variables inside a populated `mutate(...)`. NEVER write an empty mutate().\n"
            "5. FOR PROC SORT: Use `arrange()`. Use `desc()` for descending variables.\n"
            "6. FIRST. LOGIC: Use `group_by(var) %>% slice(1) %>% ungroup()`. IMPORTANT: Do NOT add an extra arrange() or sort inside this step; it must rely on the previous step's order.\n"
            "7. MACRO LOGIC: If input is a %macro, convert macro variables (&var) to column names in a mutate() call.\n"
            "8. FOR PROC FREQ: Use `df %>% count(var1, var2) %>% rename(COUNT = n)` for cross-tabs. Output MUST stay in long format.\n"
        )
    else:
        rule_set = (
            "1. Use ONLY pure Base R. DO NOT use dplyr, tidyr, or pipes (%>%).\n"
            "2. For aggregate(), ALWAYS use the formula interface.\n"
            "3. IF SAS uses DATALINES: ONLY create the data.frame. STOP immediately.\n"
            "4. IF SAS reads an existing table: start your code exactly with `df <- TABLE_NAME`.\n"
            "5. FOR PROC SORT: Use `df = df[order(...), ]`. For descending numeric, use a minus sign.\n"
            "6. FIRST. LOGIC: Use ONLY `df[!duplicated(df$var), ]`. ABSOLUTELY NO order() or sort() call allowed in this step.\n"
            "7. MACRO LOGIC: Convert macro variables (&var) to standard R object references.\n"
            "8. FOR PROC FREQ: Use EXACTLY this pattern: `df = as.data.frame(table(df$var1, df$var2))` then `names(df) = c('var1', 'var2', 'COUNT')` then `df = df[df$COUNT > 0, ]`.\n"
            "9. FOR PROC SQL GROUP BY + HAVING: WHERE filter -> aggregate separately for each output column -> merge -> HAVING filter.\n"
            "10. FOR PROC TRANSPOSE: Use reshape() with direction='long'.\n"
        )

    prompt = f"TASK: Convert this SAS step to R code.\nINPUT CONTEXT: {input_context}{env_info}\nOUTPUT: Your code must result in a final dataframe named 'df'. The last line MUST be exactly 'df'.\nSTRICT RULES:\n{rule_set}FINAL RULE: No explanations. Just executable R code.\n\nSAS STEP:\n{step}"

    try:
        raw = gemini_client.models.generate_content(model='gemini-2.0-flash', contents=prompt).text
    except Exception:
        res = groq_client.chat.completions.create(model='llama-3.3-70b-versatile', messages=[{'role':'user','content':prompt}], temperature=0)
...
def run_r_subprocess(r_code, input_df, env_dict=None):
    with tempfile.TemporaryDirectory() as d:
        ...
        full_script = ['suppressWarnings(suppressMessages(library(tidyverse)))', f'df <- read.csv("{inp_path}", stringsAsFactors=FALSE, check.names=FALSE)']
        ...
        if res.returncode != 0: raise RuntimeError(f"R Error: {res.stderr}\nCode Attempted:\n{r_code}")
...
def compare_dfs(sas_df, r_df, tol=1e-3):
    ...
    s_df, r_df_comp = sas_df.copy().reset_index(drop=True), r_df.copy().reset_index(drop=True)
    s_df.columns, r_df_comp.columns = s_df.columns.str.upper().str.strip(), r_df_comp.columns.str.upper().str.strip()

    if s_df.shape != r_df_comp.shape: return {"match": False, "details": f"Shape mismatch: SAS {s_df.shape} vs R {r_df_comp.shape}", "mismatches": []}
    
    try: r_df_comp = r_df_comp[s_df.columns]
    except KeyError as e: return {"match": False, "details": f"Missing column in R output: {e}", "mismatches": []}

    mismatches = []
    for col in s_df.columns:
        for i in range(len(s_df)):
            val_s, val_r = s_df[col].iloc[i], r_df_comp[col].iloc[i]
            try:
                if abs(float(val_s) - float(val_r)) > tol: mismatches.append({"col": col, "row": i, "sas": val_s, "r": val_r})
            except (ValueError, TypeError):
                if str(val_s).strip().upper() != str(val_r).strip().upper(): mismatches.append({"col": col, "row": i, "sas": val_s, "r": val_r})
                
    return {"match": len(mismatches) == 0, "details": "All values match!" if not mismatches else f"{len(mismatches)} values differ.", "mismatches": mismatches}

def parse_datalines(step):
    try:
        inp_match = re.search(r'input\s+(.*?);', step, re.I | re.DOTALL)
        cols = [c.replace('$', '').strip() for c in inp_match.group(1).split() if c.strip() != '$']
        ...
        return pd.DataFrame([line.split() for line in raw_lines], columns=cols)
...
def run_chain_pipeline(sas_code, uploaded_outputs, dialect):
    ...
    # 🌟 FEATURE 2: Progress Bar Init
    progress_bar = st.progress(0)

    for i, step in enumerate(steps):
        # 🌟 FEATURE 3: Step Timing Init
        ...
        if out_name_match: target_name = out_name_match.group(1).split('.')[-1].upper().strip()
        elif sort_inplace_match and not re.search(r"out\s*=", step, re.I): target_name = sort_inplace_match.group(1).split('.')[-1].upper().strip()
        else: target_name = f"STEP_{i+1}"
        ...
        if 'datalines' in step.lower() or 'cards' in step.lower(): active_df = None
        elif source_name and source_name in work_library: active_df = work_library[source_name]
        elif source_name and source_name in uploaded_outputs: active_df = uploaded_outputs[source_name]
        elif work_library: active_df = list(work_library.values())[-1]
        else: active_df = None

        res_entry = {"name": target_name, "step": step, "r_code": None, "r_output": None, "error": None, "comparison": None, "is_final": (target_name == final_ds_name), "time": 0.0}
        ...
        try:
            if 'datalines' in step.lower() or 'cards' in step.lower():
                ...
                if active_df is None and '%macro' not in step.lower(): raise ValueError(f"Input dataset '{source_name or 'WORK'}' not found.")
                r_code = call_llm_api(step, active_df.columns.tolist() if active_df is not None else [], list(work_library.keys()), dialect)
                res_entry["r_code"] = r_code
                out_df = run_r_subprocess(r_code, active_df, work_library) if active_df is not None else pd.DataFrame()
            
            work_library[target_name] = out_df
            res_entry["r_output"] = out_df
            
            if target_name in uploaded_outputs: res_entry["comparison"] = compare_dfs(uploaded_outputs[target_name], out_df)
            ...
            elif target_name == final_ds_name: res_entry["comparison"] = {"match": None, "details": "Final output reached. Upload expected CSV to validate.", "mismatches": []}
                
        except Exception as e: res_entry["error"] = str(e)
            
        # 🌟 FEATURE 3: Step Timing Save
        res_entry["time"] = time.time() - start_time
        pipeline_results.append(res_entry)
        
        # 🌟 FEATURE 2: Progress Bar Update
...
**Convert + Validate:**
1. Paste SAS code
2. Upload expected CSVs or Excels (.xlsx)
3. Run → see ✅ MATCH / ❌ MISMATCH
""")

st.subheader("📋 SAS Code")
sas_script = st.text_area("sas", height=250, label_visibility="collapsed", placeholder="Paste your SAS code here...", value=st.session_state.sas_input, key="sas_input")
...
if mode == "Convert + Execute + Validate":
    ...
    st.caption("Upload your final dataset. The app will automatically map a single uploaded file to the final step.")

    # 🌟 FEATURE 1: Excel Upload Options Enabled
    uploaded = st.file_uploader("Upload CSV or Excel files", type=["csv", "xlsx", "xls"], accept_multiple_files=True, key="uploader_" + str(st.session_state.get("upload_key", 0)))
    ...
    if uploaded:
        ...
        for i, f in enumerate(uploaded):
            ...
            except Exception as e: st.error(f"Failed to load {name}: {str(e)}")
    
    with st.expander("Or paste CSV text manually"):
        manual_name = st.text_input("Dataset name (e.g. FINAL_LABS)", key=f"manual_name_{st.session_state.get('upload_key', 0)}")
        manual_csv = st.text_area("Paste CSV here", height=100, key=f"manual_csv_{st.session_state.get('upload_key', 0)}")
...
with col_clear: st.button("🗑️ Clear", on_click=clear_all, use_container_width=True)

if run_btn:
    if not sas_script.strip(): st.warning("Paste some SAS code first."); st.stop()
    st.divider()
    ...
    if mode == "Convert Only":
        ...
        for i, step in enumerate(steps, 1):
            start_time = time.time() # 🌟 FEATURE 3: Timing
            out_name_match = re.search(r"(?:^\s*data\s+|out\s*=\s*|create\s+table\s+)([\w.]+)", step, re.I | re.M)
            sort_inplace_match = re.search(r"proc\s+sort\s+data\s*=\s*([\w.]+)", step, re.I)
            
            if out_name_match: sname = out_name_match.group(1).split('.')[-1].upper().strip()
            elif sort_inplace_match and not re.search(r"out\s*=", step, re.I): sname = sort_inplace_match.group(1).split('.')[-1].upper().strip()
            else: sname = f"Step{i}"
                ...
                with t2:
                    with st.spinner(f"Converting {sname}..."):
                        try:
                            ...
                            # 🌟 FEATURE 3: Step Timing Output
                            elapsed = time.time() - start_time
                            st.success(f"✅ {sname} converted (⏱️ {elapsed:.2f}s)")
        ...
        if all_r:
            st.divider()
            full_script_text = "\n".join(all_r)
            if "dplyr" in r_dialect: full_script_text = "library(tidyverse)\n\n" + full_script_text
        ...
        with st.spinner("Processing chain: LLM Conversion ➡️ R Execution ➡️ Data Flow..."):
            try: results = run_chain_pipeline(sas_script, uploaded_csvs, r_dialect)
            except Exception as e:
        ...
        for res in results:
            ...
            with st.expander(header, expanded=True):
                # 🌟 FEATURE 3: Step Timing Display
                st.caption(f"⏱️ Step processed in {res['time']:.2f} seconds")
                
                if res["error"]: st.error(f"Pipeline broke here: {res['error']}")
                ...
                with t2:
                    if res["r_code"]: 
                    ...
                    elif not res["error"] and res["r_output"] is not None:
                        df_r = res["r_output"]
                        col_code = []
                        for col in df_r.columns:
                            ...
                            try:
                                ...
                                if all(v == int(v) for v in floats): col_code.append(f'  {col} = c({", ".join(str(int(v)) for v in floats)})')
                                else: col_code.append(f'  {col} = c({", ".join(str(v) for v in floats)})')
                            except (ValueError, TypeError): col_code.append(f'  {col} = c({", ".join(repr(str(v)) for v in vals)})')
                ...
                with t3:
                    ...
                    else: st.info("No data output for this step.")
                with t4:
                    if cmp:
                        if cmp["match"] is True: st.success(cmp["details"])
                        elif cmp["match"] is False:
                            st.error(cmp["details"])
                            if cmp["mismatches"]: st.table(pd.DataFrame(cmp["mismatches"]).head(10))
                        else: st.warning(cmp["details"])
                    else: st.info("Intermediate step: Passed to next step automatically.")
